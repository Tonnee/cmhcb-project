...
export default function EventRegistrationForm({
  eventTitle,
  eventId,
}: EventRegistrationFormProps): React.JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createWorkshopRegistrationAction({
        ...
        workshopId: eventId || "default-event",
        workshopTitle: eventTitle,
      });
    ...
    return (
      <div className="bg-primary/5 p-8 rounded-2xl border border-primary/20 text-center animate-fade-in">
        ...
        <p className="font-sans text-sm text-light-ash">
          Thank you for registering for &ldquo;{eventTitle}&rdquo;. We will contact you soon.
        </p>
      </div>
    );
  }
