      ...
      <Container>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayPartners.map((partner) => {
            const cardContent = (
              <>
                <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center font-marcellus text-lg font-bold text-primary-dark group-hover:bg-primary group-hover:text-white transition-all duration-300 shrink-0 select-none overflow-hidden relative">
                  {partner.logo ? (
                    <img
                      src={partner.logo}
            ...
            if (partner.website) {
              return (
                <a
                  ...
                  className="relative flex items-center gap-4 bg-white rounded-3xl p-5 border border-muted/30 transition-all duration-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:border-primary-dark/40 hover:-translate-y-1 group overflow-hidden cursor-pointer"
                >
                  {cardContent}
                </a>
              );
