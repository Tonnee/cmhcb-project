...
export default function WorkshopList({ workshops }: WorkshopListProps): React.JSX.Element {
  const [searchQuery, setSearchQuery] = React.useState("");
  const [sortOrder, setSortOrder] = React.useState<"newest" | "oldest">("newest");
  const [currentPage, setCurrentPage] = React.useState(1);
  const itemsPerPage = 6;

  // Filter and sort the workshops
  const filteredAndSortedWorkshops = React.useMemo(() => {
    ...
    return result;
  }, [workshops, searchQuery, sortOrder]);
