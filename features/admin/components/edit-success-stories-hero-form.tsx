...
import { upsertSuccessStoriesHeroAction } from "@/app/(admin)/admin/actions";

export interface SuccessStoriesPageContentDB {
  id?: string;
  heroTitle: string;
...
interface EditSuccessStoriesHeroFormProps {
  initialContent?: SuccessStoriesPageContentDB | null;
}

const DEFAULT_HERO = {
  heroTitle: "Real experiences, real impact",
  heroDescription:
...
export function EditSuccessStoriesHeroForm({
  initialContent,
}: EditSuccessStoriesHeroFormProps): React.JSX.Element {
  ...
  const [ctaLabel, setCtaLabel] = React.useState(
    initialContent?.ctaLabel || DEFAULT_HERO.ctaLabel
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await upsertSuccessStoriesHeroAction(payload);
      if (res.success) {
        setSuccess(true);
        router.refresh();
      } else {
        setError(res.error || "Failed to update Success Stories page hero.");
      }
    } catch (err: unknown) {
    ...
    <div className="bg-white border border-muted rounded-2xl p-6 md:p-8 shadow-xs text-sm font-sans flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-muted pb-4">
        <div>
          <h2 className="font-marcellus text-xl font-bold text-dark-green">
            Success Stories Page Hero Header
          </h2>
          <p className="font-sans text-xs text-light-ash mt-0.5">
            ...
            >
              /success-stories <HiArrowTopRightOnSquare className="w-3 h-3" />
            </a>
          </p>
        </div>
      ...
      {success && (
        <div className="bg-emerald-50 text-emerald-800 p-4 rounded-xl text-sm font-sans font-medium border border-emerald-200 flex items-center gap-2">
          ...
          <span>Success Stories page hero banner has been successfully saved and published!</span>
        </div>
      )}
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="e.g. Real experiences, real impact"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Enter a descriptive overview of client success stories..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="font-semibold text-dark text-xs">Hero CTA Button Label</label>
              <input
                type="text"
                value={ctaLabel}
                onChange={(e) => {
            ...
            <div className="flex flex-col gap-1.5">
              <label className="font-semibold text-dark text-xs">Hero CTA Button Target URL / Anchor</label>
              <input
                type="text"
                value={ctaHref}
                onChange={(e) => {
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder="/home-review/mental-health-therapy-client-woman.png or https://..."
