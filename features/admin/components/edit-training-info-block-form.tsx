...
import { HiPhoto, HiPlus, HiTrash, HiDocumentText } from "react-icons/hi2";
import { upsertTrainingInfoBlockAction } from "@/app/(admin)/admin/actions";
...
interface EditTrainingInfoBlockFormProps {
  initialBlock?: TrainingInfoBlockDB | null;
  onClose: () => void;
  onSuccess: () => void;
}

export function EditTrainingInfoBlockForm({
  initialBlock,
  onClose,
  onSuccess,
}: EditTrainingInfoBlockFormProps): React.JSX.Element {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-h-[85vh] overflow-y-auto px-1 py-2 text-left font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder="e.g. Why Choose CMHCB Training Programs"
            className="w-full text-sm px-4 py-2.5 bg-light-ash/5 border border-muted focus:border-primary focus:bg-white rounded-xl outline-hidden transition-colors"
            required
          />
        </div>
      ...
      <div className="flex flex-col gap-3 border border-muted/50 p-4 rounded-xl bg-light-ash/5">
        <label className="text-xs font-semibold text-dark flex items-center justify-between">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 text-sm px-4 py-2 bg-white border border-muted focus:border-primary rounded-xl outline-hidden"
                required
              />
              <button
                type="button"
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center bg-light-ash/5 p-4 rounded-xl border border-muted/50">
        ...
        <div className="flex-1 flex flex-col gap-2 w-full">
          ...
          <input
            ...
            className="w-full text-xs px-3 py-1.5 bg-white border border-muted focus:border-primary rounded-lg outline-hidden"
          />
        </div>
        {isUploading && <span className="text-xs text-primary font-medium animate-pulse shrink-0">Uploading image...</span>}
      </div>
      ...
      <div className="flex justify-end gap-3 mt-4 border-t border-muted pt-4">
        <button
          type="button"
          onClick={onClose}
          className="bg-light-ash/10 hover:bg-light-ash/20 text-dark text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors cursor-pointer"
        >
          Cancel
        </button>
        <button
          ...
          className="bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-colors cursor-pointer disabled:opacity-50"
        >
