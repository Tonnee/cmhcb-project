...
const ServicesPageInputSchema = z.object({
  heroTitle: z.string().min(1, "Hero Title is required"),
  heroDescription: z.string().min(1, "Hero Description is required"),
  heroImage: z.string().min(1, "Hero Image is required"),
...
const LandingPageContentInputSchema = z.object({
  ...
  aboutStatement: z.string().optional().nullable(),
  aboutTherapistImage: z.string().optional().nullable(),
  aboutClientImage: z.string().optional().nullable(),
  aboutBrainIcon: z.string().optional().nullable(),
  aboutHeartIcon: z.string().optional().nullable(),
  aboutChartIcon: z.string().optional().nullable(),
  ...
  stat1Suffix: z.string().optional().nullable(),
  stat1Title: z.string().optional().nullable(),
  stat1Description: z.string().optional().nullable(),
  happyClientsCount: z.number().int().nonnegative(),
  stat2Suffix: z.string().optional().nullable(),
  stat2Title: z.string().optional().nullable(),
  stat2Description: z.string().optional().nullable(),
  sessionsCount: z.number().int().nonnegative(),
  stat3Suffix: z.string().optional().nullable(),
  stat3Title: z.string().optional().nullable(),
  stat3Description: z.string().optional().nullable(),
  satisfactionRate: z.number().int().min(0).max(100),
  stat4Suffix: z.string().optional().nullable(),
  stat4Title: z.string().optional().nullable(),
  stat4Description: z.string().optional().nullable(),
  ...
  trainingItem1Title: z.string().optional().nullable(),
  trainingItem1Description: z.string().optional().nullable(),
  trainingItem2Title: z.string().optional().nullable(),
  trainingItem2Description: z.string().optional().nullable(),
  trainingItem3Title: z.string().optional().nullable(),
  trainingItem3Description: z.string().optional().nullable(),
  trainingItem4Title: z.string().optional().nullable(),
  trainingItem4Description: z.string().optional().nullable(),
  appointmentHeadline: z.string().optional().nullable(),
  appointmentSubtitle: z.string().optional().nullable(),
  appointmentButtonText: z.string().optional().nullable(),
  appointmentButtonLink: z.string().optional().nullable(),
  eventsBottomText: z.string().optional().nullable(),
  eventsButtonText: z.string().optional().nullable(),
  eventsButtonLink: z.string().optional().nullable(),
  reviewCard1Title: z.string().optional(),
  reviewCard1Description: z.string().optional(),
  reviewCard2Title: z.string().optional(),
  reviewCard2Description: z.string().optional(),
  reviewPhoto1Image: z.string().optional(),
  reviewPhoto1Alt: z.string().optional(),
  reviewPhoto2Image: z.string().optional(),
  reviewPhoto2Alt: z.string().optional(),
...
export async function reorderServicesAction(
  ...
  try {
    ...
    await logActivity(
      ...
      "reorder",
      "Services Order",
      "Reordered service items display sequence."
    );
...
export async function getServicesPageContentAction(): Promise<{ success: boolean; data?: any; error?: string }> {
  try {
    const content = await prisma.servicesPageContent.findFirst();
    return { success: true, data: content };
  } catch (error: any) {
    console.error("Error in getServicesPageContentAction:", error);
    return { success: false, error: error.message || "Failed to fetch services page content" };
  }
}

export async function upsertServicesPageContentAction(
  data: any
): Promise<{ success: boolean; data?: any; error?: string }> {
  try {
    const admin = await getRequiredAdminSession();
    const validated = ServicesPageInputSchema.parse(data);
    ...
    const record = await prisma.servicesPageContent.upsert({
      ...
      create: {
        ...
        heroDescription: validated.heroDescription,
        heroImage: validated.heroImage,
        heroImageAlt: validated.heroImageAlt || "Group psychotherapeutic support session at Center for Mental Health and Care Bangladesh",
        lastUpdatedBy: admin.email,
      },
      update: {
        heroTitle: validated.heroTitle,
    ...
    await logActivity(
      ...
      record.id,
      "Services Page",
      `Updated Services page hero content`
    );
    ...
    console.error("Error in upsertServicesPageContentAction:", error);
    if (error instanceof z.ZodError) {
      return { success: false, error: error.issues.map(e => e.message).join(", ") };
    }
